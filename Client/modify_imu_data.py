...
data_acc = np.loadtxt("data/drone_acc_1589818352.txt", dtype=np.float32)
data_gyro = np.loadtxt("data/drone_gyro_1589818352.txt", dtype=np.float32)

data_acc[:,1:] = data_acc[:,1:] / 0.000597125244140625
data_gyro[:,1:] = data_gyro[:,1:] / 0.00106422515

acc_file = open("data/drone_acc_1589818352_m.txt", 'w')
gyro_file = open("data/drone_gyro_1589818352_m.txt", 'w')
